...
static BOOL EBHookClasses(NSArray *names, NSString *selector, BOOL value) {
    ...
    for (NSString *name in names) {
        if (EBHookBool(NSClassFromString(name), selector, value)) found = YES;
    }
    return found;
}
...
- (void)setURL:(NSURL *)URL {
    NSURL *rewritten = EBURL(URL);
    %orig(rewritten);
}

- (void)setValue:(NSString *)value forHTTPHeaderField:(NSString *)field {
    if (EBIsEBayHost(self.URL.host)) {
        NSString *rewritten = EBHeaderValue(field, value);
...
- (void)setAllHTTPHeaderFields:(NSDictionary *)headers {
    if (EBIsEBayHost(self.URL.host)) {
        NSDictionary *rewritten = EBHeaders(headers, self.URL);
        %orig(rewritten);
        return;
    }
    %orig;
